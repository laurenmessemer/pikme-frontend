...
import cardStackIcon from "../../assets/icons/cardstack.svg";
...
const BoostVote = ({ voteCount, submissions, onTriggerRain }) => {
    ...
    useEffect(() => {
        ...
        if (submissions.length > 0) {
            const highestVotedEntry = submissions.reduce((max, entry) => 
                voteCount[entry.id] > (voteCount[max.id] || 0) ? entry : max, submissions[0]
            );
        ...
        const newProgress = Number.isFinite(totalVotes) ? (totalVotes % 10) * 10 : 0;
    ...
    return (
        <div className="boost-vote-container">
            <div className="boost-top">
                ...
                <button className="boost-button" onClick={handleBoostActivation}>ADD</button>
            </div>
