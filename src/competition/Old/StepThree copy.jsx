...
const StepThree = ({ nextStep }) => {
  ...
  const handleConfirmAndEnter = async () => {
    ...
    try {
      // ✅ Deduct tokens first
      const tokenResponse = await axios.post("http://localhost:5004/api/competition-entry/use-token", {
        user_id: user.id,
      ...
      if (tokenResponse.data.new_balance !== undefined) {
        console.log("✅ Tokens deducted. New Balance:", tokenResponse.data.new_balance);
      ...
      await axios.post("http://localhost:5004/api/competition-entry/enter", {
        user_id: user.id,
        contest_id: contestId,
      ...
      console.error("❌ Error processing entry:", error.response?.data || error);
      setError(error.response?.data?.message || "Failed to process competition entry.");
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <div className="step-three-container flex">
      {showInsufficientFunds && <InsufficientFunds onClose={() => setShowInsufficientFunds(false)} />}
