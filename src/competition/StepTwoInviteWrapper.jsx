...
const StepTwoInviteWrapper = () => {
  ...
  if (user === undefined) return <p className="loading-message">🔐 Checking authentication...</p>;
  if (!user) return <Navigate to={`/login?redirect=/join/upload/${inviteCode}`} />;
  if (loading) return <p className="loading-message">🔄 Loading contest...</p>;
  if (error) return <p className="error-message">❌ {error}</p>;

  return <StepTwoInvite nextStep={() => navigate(`/join/upload/${inviteCode}/confirm`)} />;
};
